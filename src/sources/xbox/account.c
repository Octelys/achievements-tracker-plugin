...
static obs_properties_t *source_get_properties(void *data) {
    ...
    if (s && s->source) {
        ...
        if (settings) {
            ...
            obs_data_set_string(settings,
                                "xbox_manage_note",
                                "Open Tools -> Xbox Account to sign in or sign out globally.");
    ...
    obs_property_t *note_prop = obs_properties_add_text(p, "xbox_manage_note", "Manage account", OBS_TEXT_INFO);
    obs_property_text_set_info_type(note_prop, OBS_TEXT_INFO_NORMAL);
    obs_property_text_set_info_word_wrap(note_prop, true);
