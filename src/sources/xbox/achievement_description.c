...
static void on_connection_changed(bool is_connected, const char *error_message) {
    ...
    update_achievement_description(g_last_unlocked);
}
 ...
 * newly unlocked achievement's description.
 *
 ...
 * - Showing the last unlocked achievement description for 60 seconds
 * - Showing random locked achievement descriptions (15 seconds each) for 60 seconds total
 */
    ...
    case DISPLAY_PHASE_LAST_UNLOCKED:
        ...
        if (g_phase_timer <= 0.0f) {
            ...
            if (count_locked_achievements(achievements) > 0) {
                ...
                if (locked) {
                    update_achievement_description(locked);
                }
            } else {
