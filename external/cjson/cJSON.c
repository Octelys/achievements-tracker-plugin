...
cJSON *cJSON_ParseWithOpts(const char *value, const char **return_parse_end, int require_null_terminated)
{
    ...
    if (require_null_terminated && *end) { cJSON_Delete(c); return NULL; }
...
cJSON *cJSON_CreateTrue(void) { cJSON *i = cJSON_New_Item(); if (i) i->type = cJSON_True; i->valueint = 1; return i; }
...
cJSON *cJSON_CreateString(const char *string) { cJSON *i = cJSON_New_Item(); if (i) { i->type = cJSON_String; if (string) { size_t l = strlen(string); i->valuestring = (char*)cjson_malloc(l + 1); if (i->valuestring) memcpy(i->valuestring, string, l + 1); } } return i; }
cJSON *cJSON_CreateRaw(const char *raw) { cJSON *i = cJSON_New_Item(); if (i) { i->type = cJSON_Raw; if (raw) { size_t l = strlen(raw); i->valuestring = (char*)cjson_malloc(l + 1); if (i->valuestring) memcpy(i->valuestring, raw, l + 1); } } return i; }
...
cJSON *cJSON_GetObjectItem(const cJSON * const object, const char * const string)
{
    cJSON *c = object ? object->child : NULL;
    while (c && c->string) {
        if (strcasecmp(c->string, string) == 0) return c;
        c = c->next;
    }
    return NULL;
}
...
static char *print_array(const cJSON *item)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!entries[i]) { /* cleanup */
            for (int j = 0; j < i; j++) cjson_free(entries[j]);
            cjson_free(entries);
            return NULL;
        }
    ...
    if (!out) {
        for (int i = 0; i < n; i++) cjson_free(entries[i]);
        cjson_free(entries);
        return NULL;
    }
...
static char *print_object(const cJSON *item)
{
    ...
    if ((!entries || !names) && n) return NULL;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!names[i] || !entries[i]) {
            for (int j = 0; j <= i; j++) {
                if (names) { if (names[j]) cjson_free(names[j]); }
                if (entries) { if (entries[j]) cjson_free(entries[j]); }
            }
    ...
    char *out = (char*)cjson_malloc(len + 1);
    if (!out) {
        for (int i = 0; i < n; i++) { cjson_free(names[i]); cjson_free(entries[i]); }
        cjson_free(names);
        cjson_free(entries);
        return NULL;
    }
...
cJSON *cJSON_Duplicate(const cJSON *item, int recurse) { (void)item; (void)recurse; return NULL; }
void cJSON_InsertItemInArray(cJSON *array, int which, cJSON *newitem) { (void)array;(void)which;(void)newitem; }
void cJSON_ReplaceItemViaPointer(cJSON * const parent, cJSON * const item, cJSON *replacement) { (void)parent;(void)item;(void)replacement; }
void cJSON_ReplaceItemInArray(cJSON *array, int which, cJSON *newitem) { (void)array;(void)which;(void)newitem; }
void cJSON_ReplaceItemInObject(cJSON *object, const char *string, cJSON *newitem) { (void)object;(void)string;(void)newitem; }
void cJSON_ReplaceItemInObjectCaseSensitive(cJSON *object, const char *string, cJSON *newitem) { (void)object;(void)string;(void)newitem; }
void cJSON_AddItemReferenceToArray(cJSON *array, cJSON *item) { (void)array;(void)item; }
void cJSON_AddItemReferenceToObject(cJSON *object, const char *string, cJSON *item) { (void)object;(void)string;(void)item; }
cJSON *cJSON_DetachItemViaPointer(cJSON *parent, cJSON * const item) { (void)parent;(void)item; return NULL; }
cJSON *cJSON_DetachItemFromObject(cJSON *object, const char *string) { (void)object;(void)string; return NULL; }
cJSON *cJSON_DetachItemFromObjectCaseSensitive(cJSON *object, const char *string) { (void)object;(void)string; return NULL; }
void cJSON_DeleteItemFromObject(cJSON *object, const char *string) { (void)object;(void)string; }
void cJSON_DeleteItemFromObjectCaseSensitive(cJSON *object, const char *string) { (void)object;(void)string; }
cJSON *cJSON_CreateIntArray(const int *numbers, int count) { (void)numbers;(void)count; return NULL; }
cJSON *cJSON_CreateFloatArray(const float *numbers, int count) { (void)numbers;(void)count; return NULL; }
cJSON *cJSON_CreateDoubleArray(const double *numbers, int count) { (void)numbers;(void)count; return NULL; }
cJSON *cJSON_CreateStringArray(const char **strings, int count) { (void)strings;(void)count; return NULL; }
